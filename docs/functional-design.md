...
- 編集画面のプロジェクト設定からFPSを1～60の整数で変更できる。変更は通常の自動保存対象とし、新しいrevisionへ保存する。
...
- 移動
- サイズ変更（四隅のハンドルは縦横比を維持し、上下左右の辺中央ハンドルは反対側の辺を固定して片方向だけ拡大・縮小する）
- 回転
- 透明度
- 前面・背面への移動
- 表示区間
...
- キャンバス上の画像を右クリックし、「画像をダウンロード」から参照元の画像素材を端末へ保存できる。
- 対応ブラウザでは保存先選択ダイアログを表示する。未対応ブラウザではブラウザ既定のダウンロード先へ保存する。
- ロック中の画像もダウンロードできる。移動、拡大・縮小、回転、反転などのキャンバス上の変形はダウンロードファイルへ反映しない。
- ダウンロード時は元のファイル名を優先し、ファイル名がない生成画像には素材名とMIMEタイプに対応する拡張子を使用する。
- 素材一覧のページ上限を超える古い画像でも、表示中レイヤーの素材IDと名前を使ってダウンロードできる。
...
- 作成時に「テキストボックス（横）」または「テキストボックス（縦）」を選択する。
- 横書き・縦書きのどちらも、キャンバス上のテキストをダブルクリックして、改行を含む本文を直接編集できる。`Ctrl+Enter`（macOSでは`Cmd+Enter`）またはフォーカス移動で確定し、`Escape`でキャンセルする。
- 縦書きの改行は右から左へ新しい列を追加する。列が現在のテキストボックス幅に収まらない場合は、右端の位置を保ったまま左方向へ幅を自動拡張し、改行後の文字をクリップしない。
- テキストボックスの右クリックメニューから、文字方向、フォント、文字サイズ、文字色を変更できる。
- ネオングラデーションを有効化でき、ネオン色の既定値は淡い青`#9bdcff`とする。文字色とネオン色は個別に変更できる。
- 表示エフェクトは一括表示と文字送りを選択できる。文字送りは1秒あたりの文字数を設定でき、レイヤーの表示開始時刻を基準にシーク・再生・書き出しで同じ表示結果になる。
- フォントは書き出し環境に存在する安全な候補から選択し、存在しない場合は`serif`、`sans-serif`または`monospace`へフォールバックする。
- 選択中のテキストボックスは`Ctrl+C`（macOSでは`Cmd+C`）でコピーし、`Ctrl+V`（macOSでは`Cmd+V`）で同じ位置、サイズ、スタイル、表示区間へ複製する。複製したテキストボックスは独立したレイヤートラックへ追加する。
- テキスト入力欄にフォーカスがある場合は通常の文字コピー・貼り付けを優先し、画像貼り付けを含む他のクリップボード操作を妨げない。

### 9.3 `TML-001` 動画時間・タイムライン

- 動画時間の初期値は5秒、最小値は0.1秒とする。編集精度は50ミリ秒とし、5秒単位には制限しない。
- 利用者は再生ヘッドを任意位置へ移動し、「再生位置で終了」から後方の不要区間をカットできる。カット時は後方にあるレイヤー、キーフレーム、テロップ、音声、カメラエフェクトを削除または終端位置で切り詰める。
- 利用者は「区間を削除」を選び、時間目盛り上をドラッグして任意の開始・終了位置を指定できる。確定すると選択区間を削除し、後続のテロップ、画像・テキスト・図形、キーフレーム、音声、カメラエフェクトを削除時間分だけ前へ詰める。
- 選択区間をまたぐ音声クリップは前後へ分割し、後半の素材再生位置を削除時間に合わせて進める。区間削除はUndo・Redoの1操作として扱う。
- 利用者は「区画追加」を選び、時間目盛りで挿入位置、数値欄で追加時間を指定できる。確定すると全トラックへ空き区間を挿入し、挿入位置以降のクリップ、テロップ、キーフレーム、音声、カメラエフェクトを同じ時間だけ後ろへ移動する。
- 挿入位置をまたぐクリップは前後へ分割して同一レイヤーとして保持し、音声後半は素材の再生位置を維持する。区画追加はUndo・Redoの1操作として扱う。
- タイムライン右端の終了ハンドルを左右へドラッグし、動画時間を1秒単位で任意に延長・短縮できる。短縮時は「再生位置で終了」と同じカット処理を適用する。
- タイムラインは開閉でき、上端のハンドルを上方向へドラッグして表示領域を拡大できる。
- 秒目盛りと各レコードの縦罫線は同じ時間軸へ揃え、レイヤーバーの移動・伸縮中は開始時刻と終了時刻を表示する。
- 1つのレイヤートラックには画像・テキスト・図形のクリップを複数配置できる。クリップの右クリックから隣接トラックへまとめる、または別トラックへ分離する。
- 隣接トラックへまとめたクリップは既存クリップの終了後へ自動配置し、必要に応じて動画時間を延長する。各クリップの表示区間とアニメーションは独立して保持する。
- レイヤーの表示終了、キーフレーム、テロップ、音声の終了が現在の動画時間を超える場合は、その内容が収まる50ミリ秒単位まで自動延長する。
- 音声クリップは左右端をドラッグして素材の使用開始位置と終了位置をトリムできる。クリップ本体のドラッグはタイムライン上の再生開始位置を変更する。
- 再生位置、プレビューのループ終端、タイムライン目盛り、MP4のフレーム数は同じ動画時間計算を利用する。
- プレビュー再生中は選択枠、リサイズ・回転ハンドル、ロック表示などの編集用オーバーレイを非表示にし、停止後に選択状態を復元する。
...
- 音量と前後余白を設定する。
- AivisSpeechの話者・スタイルと読み上げ文を指定してWAVを生成できる。
- 話速、感情表現、テンポの緩急、生成音量を調整できる。
- 生成したWAVはユーザー専用の音声素材として保存し、現在の再生位置へナレーションクリップとして追加する。
- ナレーションとテロップを一致させる場合は、読み上げ文をテロップ単位で個別合成して連結し、各区間の実測WAV長を表示時間に使用する。
- AIが動画全体を作成する場合、個別合成した音声は動画全体で1本のマスターナレーションへ連結し、音声フレーム境界からテロップ、画像区間、動画尺を一括算出する。
- テロップ表示文と音声合成用の読み上げ文を分離し、表示を変えずに固有名詞や多義語の読みを指定できる。
- 一括作成はProject Document全体を検証後、1回のRevisionとして保存し、途中状態を保存しない。
- 同期検証では、音声素材に保存した本文・区間開始終了時刻とテロップを照合する。構造検証や静止画確認だけでは同期済みと判定しない。
- Engine未接続、生成失敗、不正な音声応答を区別して安全なエラーを表示する。

BGM:

- MP3をタイムラインへ直接ドラッグ＆ドロップして、ドロップした時刻へ音声クリップを追加する。
- プロジェクト全体または指定区間へ配置する。
- 音声バーのドラッグまたは数値入力で再生開始位置を調整する。
- 音量、ループ、フェードイン・アウトを設定し、プレビュー再生へ即時反映する。
- ナレーション中に簡易ダッキングできる。
...
5. 所有権と実ファイルを検証した画像素材をレンダラー専用URLへ対応付ける。
6. Scene Rendererでフレームを生成する。
7. FFmpegで映像・音声を合成する。
8. 一時ファイルをffprobe等で検証する。
9. 完成ファイルをユーザー領域へ確定保存する。
10. exportを成功状態に更新する。
11. ユーザーへ完了通知する。

- Scene RendererはProject Documentの幅・高さを出力フレームへそのまま適用し、16:9と9:16のどちらも指定解像度で書き出す。
- フレーム生成前にRender Planを作成し、背景、画像、図形、全文表示済みテキストなど、視覚状態が変化しない区間は先頭フレームを1回だけ描画して再利用する。
- キーフレーム、カメラエフェクト、フェード、文字送りなど時間依存の区間は従来どおり全フレームを描画する。未知のレイヤー型・効果は動的として扱う。
- 静止区間の最適化は映像生成だけへ適用し、ナレーション、BGM、効果音は区間分割せず最後に1回だけ合成する。
- 最適化を無効化して全フレーム描画へ戻せるようにし、完成後はffprobeで総フレーム数、FPS、映像尺、音声尺を検証する。
- 対応ブラウザでは、編集プレビューと同じWebGL描画器を使用するブラウザ内MP4書き出しを試験利用できる。WebGL2、WebCodecsまたは要求コーデックが使えない場合も、従来のサーバー書き出しを利用できる。
- ブラウザ内書き出しは進捗とキャンセルを表示し、映像はH.264、音声がある場合はAACとして同じMP4へ格納する。
- 書き出し開始前にファイル名、解像度、FPSを確認・変更できる。省略時は対象revisionの動画設定を使用し、書き出し画面での変更は当該exportだけへ適用してProject Documentを変更しない。
- 画像バイナリはProject JSONへBase64で埋め込まず、ページへ公開しない検証済みローカルパスからレンダラーが必要時に配信する。
- フレーム生成中は生成済みフレーム数に応じた進捗をジョブへ通知し、FFmpeg処理開始時にも進捗とheartbeatを更新する。
...
レンダラーが異常終了した場合は、終了コードと標準エラーまたは標準出力の安全な末尾を内部エラーログへ保存する。診断出力がない場合も空文字にはせず、その旨を記録する。
...
編集画面と外部RESTクライアントは同じAssistant Service、Orchestrator、Tool Registryを利用する。Personal API Tokenでは`assistant:read`または`assistant:write`を必須とし、Cookie認証時は従来どおりCSRFを検証する。

| Method | Path | 機能 |
| --- | --- | --- |
| GET/POST | `/api/v1/projects/{project_id}/assistant/threads` | 会話一覧・作成 |
| GET | `/api/v1/projects/{project_id}/assistant/threads/{thread_id}` | 会話、実行、ツール履歴取得 |
| POST | `/api/v1/projects/{project_id}/assistant/threads/{thread_id}/messages` | メッセージ送信・AI実行開始 |
| GET | `/api/v1/projects/{project_id}/assistant/runs/{run_id}` | 実行状態取得 |
| GET | `/api/v1/projects/{project_id}/assistant/runs/{run_id}/events` | SSEイベント取得 |
| POST | `/api/v1/projects/{project_id}/assistant/tool-calls/{call_id}/approve` | ツール承認 |
| POST | `/api/v1/projects/{project_id}/assistant/tool-calls/{call_id}/reject` | ツール却下 |
| POST | `/api/v1/projects/{project_id}/assistant/runs/{run_id}/cancel` | 実行取消 |
| POST | `/api/v1/projects/{project_id}/assistant/runs/{run_id}/undo` | AI変更のUndo |
