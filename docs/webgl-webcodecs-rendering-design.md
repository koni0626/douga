...
長尺プロジェクトでも音声再生とポインター操作を阻害しないよう、編集プレビューでは次を守る。

- Reactへ公開する再生時刻は壁時計を基準に最大10Hzで更新し、`requestAnimationFrame`ごとの画面全体再描画を行わない。
- 画像素材の取得・デコード・GPU転送は、参照する素材IDまたはURLが変わった場合だけ実行する。
- テロップのタイムライン計算と、同じ文字・スタイルのCanvas 2Dラスタライズ結果を再利用する。
- 音声要素は再生位置の15秒前から準備し、終了から1秒を過ぎた要素を解放する。ループまたは長さ未確定の音声は開始後も保持する。
- 通常再生中はブラウザの音声クロックを優先する。再生開始・シーク・ループ折り返し時、または1秒ごとの確認で200msを超えるずれがある場合だけ再同期する。

これらは編集プレビュー専用の負荷制御であり、WebCodecsおよびサーバーFFmpegの書き出し時刻・出力FPSは変更しない。
