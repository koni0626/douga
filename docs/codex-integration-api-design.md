...
- 外部クライアントによる作成系POSTでは`Idempotency-Key`を必須とする。
...
`202 Accepted`で`preview_id`と`job_id`を返す。クライアントは状態取得APIをポーリングする。初期実装ではWebhookを使用しない。
...
NovelCreatorリポジトリへ小さなPythonクライアントを置く。
