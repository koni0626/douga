...
| `P7-016` | 動画全体のマスターナレーション連結・フレーム境界生成 | L |
| `P7-017` | ナレーション付き動画の一括Document構築・原子的Revision保存 | XL |
| `P7-018` | 音声再生成時のテロップ・画像・Keyframe自動リップル | L |
| `P7-019` | 表示文・読み上げ文分離とProject読み辞書 | M |
| `P7-020` | 単一マスターナレーション前提のプレビュー同期回帰テスト | L |
...
- AI一括作成ではテロップ単位の音声を動画全体で1本へ連結し、画像、テロップ、音声、動画尺を1回のRevisionで同期できる。
- 表示テロップを変えずに音声合成用の読みを指定できる。
...
| `P9-017` | 静止区間Render Plan・時間依存判定・Fingerprint | L |
| `P9-018` | 静止PNG再利用・同期描画API・legacy fallback | L |
| `P9-019` | FFconcat duration manifest技術検証・一時容量制御 | M |
| `P9-020` | legacy/hybrid映像・音声同期・性能回帰テスト | L |
| `P9-021` | WebGL2共通描画器・SVGフォールバック | L |
| `P9-022` | WebCodecs H.264/AAC・MP4ブラウザ書き出し試験版 | L |
| `P9-023` | SVG/WebGL画像差分・長尺メモリ・端末互換性検証 | L |
...
- 静止区間を1回だけChromiumで描画し、アニメーション区間だけを毎フレーム描画できる。
- legacyとhybridで総フレーム数、動画尺、代表フレーム、音声開始位置が一致する。
- 未知の視覚効果を静止と誤判定せず、最適化を無効化してlegacy方式へ戻せる。
- WebGLプレビューとブラウザ内MP4書き出しが同じフレーム描画器を使用し、非対応環境ではSVGまたはサーバーFFmpegへ戻せる。
